...
export default mutation(async ({ db, auth }, goatId: Id<'goats'>) => {
  const user = await getUserDoc(db, auth);
  const goatDoc = await db.get(goatId);
  if (goatDoc === null) {
    throw 'How did you vote for a goat that did not exist?.';
  } else {
    ...
    if (existingIds.indexOf(user._id.id) < 0) {
      ...
      console.log(
        `successfully voted! This goat now has ${goatDoc.voters.length} votes`
      );
    } else {
      console.log(
        `Current user ${user._id} has already voted for ${goatDoc.text}`
      );
    }
  }
});
