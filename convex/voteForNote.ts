...
export default mutation(async ({ db, auth }, noteId: Id<'notes'>) => {
  ...
  if (noteDoc === null) {
    throw 'How did you vote for a note that did not exist?.';
  } else {
    if (!user.noteVotes) {
    ...
    if (user.noteVotes.has(noteDoc._id.id)) {
      console.log(
        `Current user ${user._id} has already voted for ${noteDoc.text}`
      );
      return;
    }
    ...
    console.log(
      `successfully voted! This user has now voted for ${user.noteVotes.size} notes`
    );
  }
});
